...
export default function AdminHomeServices() {
  ...
  const createItem = async () => {
    ...
    await api.post("/homeservices", fd, {
    ...
    alert("Home Service created");
  };

  // DELETE
  const deleteItem = async (id) => {
    if (!window.confirm("Delete this?")) return;
    await api.delete(`/homeservices/${id}`);
  };
  ...
  const saveEdit = async (id) => {
    ...
    await api.put(`/homeservices/${id}`, fd, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  ...
  return (
    <div className="p-6 bg-white rounded shadow w-full">
      <h2 className="font-bold text-xl mb-4">Home Services Management</h2>

      {/* CREATE */}
      <div className="border p-4 rounded mb-6">
        <h3 className="font-semibold mb-3">Create New Home Service</h3>

        <input
          className="border p-2 w-full mb-2 rounded"
          placeholder="Service Name (max 30 characters)"
          maxLength={30}
          value={name}
        ...
        <input
          className="border p-2 w-full mb-2 rounded"
          placeholder="Phone Number (max 28 characters)"
          maxLength={28}
          value={phone}
        ...
        <div key={item._id} className="border p-3 rounded mb-3 bg-gray-50">
          {editingId === item._id ? (
            <>
              ...
              <textarea
                className="border p-2 w-full mb-2 rounded"
                placeholder="Descrption (max 50 characters)"
                maxLength={50}
                rows="2"
              ...
              <input
                className="border p-2 w-full mb-2 rounded"
                placeholder="Phone Nunmber (max 28 characters)"
                maxLength={28}
                value={editPhone}
