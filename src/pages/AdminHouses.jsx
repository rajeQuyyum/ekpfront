...
export default function AdminHouses() {
  ...
  const createItem = async () => {
    ...
    setImage(null);
    setRating(0);
    alert("House/Hostel created");
  };

  // DELETE
  const deleteItem = async (id) => {
    if (!window.confirm("Delete this house/hostel?")) return;
  ...
  return (
    <div className="p-6 bg-white rounded shadow w-full">
      <h2 className="font-bold text-xl mb-4">Houses / Hostels Management</h2>

      {/* CREATE */}
      <div className="border p-4 rounded mb-6">
        <h3 className="font-semibold mb-3">Create New House / Hostel</h3>

        <input
          className="border p-2 w-full mb-2 rounded"
          placeholder="House/Hostel Name (max 30 charaters)"
          maxLength={30}
          value={name}
        ...
        <div key={item._id} className="border p-3 rounded mb-3 bg-gray-50">
          {editingId === item._id ? (
            <>
              ...
              <input
                className="border p-2 w-full mb-2 rounded"
                placeholder="Phone Number (max 50 charaters)"
                maxLength={28}
                value={editPhone}
